        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            ...
            >
              {/* Logo */}
              <motion.div 
                className="w-20 h-20 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl mx-auto mb-8 flex items-center justify-center shadow-lg shadow-emerald-200"
              ...
              <h1 className="text-3xl font-bold text-gray-900 mb-3">
                Bem-vindo ao <span className="text-emerald-600">HeartBalance</span>
              </h1>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-emerald-100">
              ...
              <Button
                onClick={handleStart}
                className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white py-6 rounded-2xl text-lg font-medium shadow-lg shadow-emerald-200 transition-all"
              >
                Iniciar Quiz
              ...
              {questions[step - 1].type === 'number' ? (
                <div className="space-y-4">
                  <Input
                    ...
                    className="w-full py-6 text-lg rounded-xl border-gray-200 focus:border-emerald-500 focus:ring-emerald-500"
                    onKeyPress={(e) => e.key === 'Enter' && handleNumberSubmit()}
                  />
                  <Button
                    onClick={handleNumberSubmit}
                    disabled={!inputValue || parseInt(inputValue) <= 0}
                    className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-6 rounded-xl text-lg"
                  >
                    Continuar
                    <ArrowRight className="w-5 h-5 ml-2" />
                  </Button>
                ...
                <div className="space-y-3">
                  {questions[step - 1].options.map((option) => (
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full p-5 text-left rounded-xl border-2 border-gray-100 hover:border-emerald-500 hover:bg-emerald-50 transition-all flex items-center justify-between group"
                    >
                      <span className="text-lg text-gray-800 font-medium">{option}</span>
                      <div className="w-8 h-8 rounded-full border-2 border-gray-200 group-hover:border-emerald-500 group-hover:bg-emerald-500 flex items-center justify-center transition-all">
